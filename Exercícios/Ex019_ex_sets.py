...
while sair != 'nao': #Enquanto sair for diferente de nao faça:
    estados_sami.add(input('Qual Estado Sami visitou a mais? ')) #Adiciono o elemento ao conjunto, lembrando que se for elemento repetido ele ignora.
...
#Difference imprime a diferença entre o conjunto de dudu e sami, logo resta apenas os elementos que apenas estã em dudu
...
#A função len retorna a quantidade de elementos dentro do conjunto, logo a quantidade de Estados visitados
if len(estados_sami) > len(estados_dudu):
    #O calculo len(estados_sami)*100//27 apresenta a porcentagem dos estados visitados em relação ao total
